...
**Bank Marketing** dataset from the UCI Machine Learning Repository. It
covers end-to-end model building, evaluation, and deployment using a Streamlit
web app, as required in the assignment instructions.
...
phone to promote long-term term deposit products to its clients.
...
inconsistent. The goal of this project is to build and compare
...
and macro-economic indicators. These models can help the bank
prioritise high-probability clients, improve campaign efficiency, and reduce
unnecessary calls to clients who are unlikely to subscribe.
...
**UCI Machine Learning Repository**. It contains data from
...
- **20 input attributes** plus **1 target variable**.
...
- **Client information**
  ...
  - `job`: Type of job (admin., blue-collar, technician, services, etc.).
  - `marital`: Marital status (married, single, divorced, etc.).
...
- `y`: Whether the client subscribed to a term deposit at the end of the
  campaign (binary: **"yes"** or **"no"**). In the modelling
...
All six models listed in the assignment are implemented on the same dataset:
